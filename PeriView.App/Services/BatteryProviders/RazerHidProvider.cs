...
/// <summary>
/// 雷蛇HID设备电池提供者
/// 参考蓝牙项目中的 D.x 类（支持雷蛇设备）
/// </summary>
public sealed class RazerHidProvider : IBatteryProvider, IDisposable
{
    ...
    public async Task ReadBatteryAsync(IBatteryProviderContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var hidDevices = deviceList.GetHidDevices()
                .Where(d => d.VendorID == 14621)
                .ToList();
            
            if (hidDevices.Count == 0)
            {
                Logger.Debug("未找到雷蛇HID设备");
                return;
            }
            
            Logger.Debug($"找到 {hidDevices.Count} 个雷蛇HID设备");
        ...
        {
            Logger.Error("读取雷蛇HID设备电池信息失败", ex);
        }
    }
    ...
    public async Task ReadBatteryAsync(IBatteryProviderContext context, string deviceId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (hidDevices.Count == 0)
            {
                Logger.Debug($"未找到设备ID为 {deviceId} 的雷蛇HID设备");
                return;
            }
        ...
        {
            Logger.Error($"读取雷蛇HID设备 {deviceId} 电池信息失败", ex);
        }
    }
    ...
    private async Task ReadDeviceBattery(HidDevice device, IBatteryProviderContext context, CancellationToken cancellationToken)
    {
        ...
        Logger.Debug($"尝试读取雷蛇设备电池: {displayName} ({deviceKey})");
        ...
        try
        {
            ...
            if (batteryInfo.HasValue)
            {
                ...
                Logger.Debug($"雷蛇设备电池读取成功: {displayName}, 电量: {percent}%, 充电: {isCharging}, 休眠: {isSleeping}");
            }
            else
            {
                Logger.Debug($"无法读取雷蛇设备电池信息: {displayName}");
            }
        }
        catch (Exception ex)
        {
            Logger.Error($"读取雷蛇设备电池信息异常: {displayName}", ex);
        }
    }
    ...
    private string GetDeviceDisplayName(HidDevice device)
    {
        ...
        return $"雷蛇设备 (VID: {device.VendorID:X4}, PID: {device.ProductID:X4})";
    }
    ...
    {
        return $"razer_hid_{device.VendorID:X4}_{device.ProductID:X4}_{GetStableDeviceId(device.DevicePath)}";
    }
    ...
    private void CleanupRemovedDevices(HashSet<string> currentDeviceKeys)
    {
        ...
        foreach (var key in removedKeys)
        {
            ...
            {
                Logger.Debug($"雷蛇设备已移除: {device.DisplayName}");
